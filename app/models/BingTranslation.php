...
class BingTranslation {
    ...
    public function __construct($cid, $secret) {
        ...
        \Log::info('contruct :' . $this->clientID . ' ' . $this->clientSecret);
    }

    public function get_access_token() {
        \Log::info('get_access_token() :' . $this->clientID . ' ' . $this->clientSecret);
        ...
        \Log::info('get_access_token()  postParams :' . $postParams);
        $ch = curl_init();
        ...
        \Log::info('get_access_token()  strResponse :' . $strResponse);
        \Log::info('get_access_token()  result      :' . $result);
        // Decode the returned JSON string.
        // $objResponse = json_decode($result,true);
        // $objResponse = json_decode(utf8_encode($result));
        // $objResponse = html_entity_decode($result);
        
        $result = str_replace('&quot;', '"', $result);
        $result = str_replace('\\', '', $result);
        
        \Log::info('get_access_token()  result2     :' . $result);
